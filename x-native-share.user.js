...
(function () {
  ...
  var TAG = '[x-native-share]';
  var MARKER_ATTR = 'data-tm-x-native-share';
  console.info(TAG, 'v0.3.0');
  ...
  function isShareTrigger(node) {
    ...
    // Primary: aria-label-based (current X on most platforms)
    if (node.matches('button[aria-label="Share post" i], button[aria-label*="share post" i]')) return true;
    // Secondary: data-testid-based
    if (node.matches('[data-testid="share"], [data-testid="Share"]')) return true;
    return false;
  }

  // Scan for share menus. X currently uses [role="menu"]; legacy fallback [data-testid="Dropdown"] kept.
  function scanMenus() {
  ...
  function tryInject(container) {
    // Skip if we already injected into this container
    if (container.querySelector('[' + MARKER_ATTR + ']')) return;

    var template = container.querySelector('[role="menuitem"]');
    if (!template) {
      console.info(TAG, 'container found (' + (container.getAttribute('role') || container.getAttribute('data-testid') || 'unknown') + ') but no menuitem yet');
      return;
    }
    ...
    console.info(TAG, 'menu item injected');
  }
  ...
  function onMenuItemClick(event) {
    ...
    console.info(TAG, 'share payload ready:', data.title, '(' + data.text.length + ' chars)');
  ...
  function pickArticle() {
    // Use cached article if recent and still connected
    ...
    // Status page: focused tweet has tabindex="-1"
  ...
  function extractText(container) {
    ...
    // 1. X Articles — rich text view (long-form posts with title)
    ...
    // 3. Fallback — whatever text is in the container
  ...
  function resolvePostUrl(article) {
    ...
    // Fallback to current URL if on a status/article page
  ...
  // Initial scan in case a menu is already open
  scanMenus();
